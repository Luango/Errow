	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag (v2f i) : SV_Target {
				float xL = tex2D(_XTex, i.uv + half2(-1, 0) * _XTex_TexelSize).x;
				float xR = tex2D(_XTex, i.uv + half2(1, 0) * _XTex_TexelSize).x;
				float xT = tex2D(_XTex, i.uv + half2(0, 1) * _XTex_TexelSize).x;
				float xB = tex2D(_XTex, i.uv + half2(0, -1) * _XTex_TexelSize).x;
				float xC = tex2D(_XTex, i.uv).x;
				...
				float bC = tex2D(_BTex, i.uv).x;
				
			    float result = (xL + xR + xT + xB + _Alpha * bC) * _rBeta;
