	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag (v2f i) : SV_Target {
				float3 xL = tex2D(_XTex, i.uv + half2(-1, 0) * _XTex_TexelSize).xyz;
				float3 xR = tex2D(_XTex, i.uv + half2(1, 0) * _XTex_TexelSize).xyz;
				float3 xT = tex2D(_XTex, i.uv + half2(0, 1) * _XTex_TexelSize).xyz;
				float3 xB = tex2D(_XTex, i.uv + half2(0, -1) * _XTex_TexelSize).xyz;
				float3 xC = tex2D(_XTex, i.uv).xyx;
				...
				float3 bC = tex2D(_BTex, i.uv).xyz;
				
			    float3 result = (xL + xR + xT + xB + _Alpha * bC) * _rBeta;
